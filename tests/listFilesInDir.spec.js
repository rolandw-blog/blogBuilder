...
const getRoutePositionInDir = require(path.resolve(
	buildDir,
	"getRoutePositionInDir.js"
));
const getLinkToHtmlFilepath = require(path.resolve(
	buildDir,
	"getLinkToHtmlFilepath.js"
));

describe("Test listFilesInDir", () => {
	test("Check if a path exists", () => {
		expect(listFilesInDir("tests/testViews")).toEqual([
			"about.js",
			"index.js",
		]);
...
describe("Test getRoutePositionInDir", () => {
	test("get Route Position In Dir", () => {
		...
		expect(
			getRoutePositionInDir(path.resolve(viewsDir, "notes"), "page3.js")
		).toEqual(0);
	});
});
...
describe("Test getting html filepath", () => {
	test("Check if a path exists", () => {
		...
		expect(
			getLinkToHtmlFilepath({ path: "notes/page3.js" }, "page3")
		).toEqual("/notes/page3");
		...
		expect(getLinkToHtmlFilepath({ path: "" }, "about.js")).toEqual(
			"/about"
		);
	});
});
